...
def plot_training_results(df, model_name, savefig=False):
...
def train_validate_model(model, num_epochs, model_name, criterion, optimizer, 
                         ...
                         output_path = '.', savefig=False, logger=None):
    ...
    for epoch in range(num_epochs):
        logger.info(f"Starting {epoch + 1} epoch ...")
        ...
        logger.info(f'Epoch: {epoch+1}, trainLoss:{train_loss:6.5f}, validationLoss:{validation_loss:6.5f}, {metric_name}:{validation_metric: 4.2f}')
    ...
    plot_training_results(results, model_name, savefig=savefig)
    return results
...
def visualize_predictions(model : torch.nn.Module, dataSet : Dataset,  
        ...
        id_to_color : np.ndarray, savefig : bool = False):
    ...
    if savefig:
        plt.savefig("artifacts/visualize.png")
    else:
        plt.show()